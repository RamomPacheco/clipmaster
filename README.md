...
2. **FFmpeg** (required for development unless you use bundled binaries)
   - **Option A — bundled with the project:** copy `ffmpeg.exe` and `ffprobe.exe` to `bundled/ffmpeg/windows/` (or run `scripts\fetch_ffmpeg_windows.ps1`).
   - **Option B — system install:** https://ffmpeg.org/download.html or `choco install ffmpeg`; verify with `ffmpeg -version`.
...
**Quick setup (recommended on first run):**
```powershell
cd clipmaster
...
3. **Install dependencies** (use the venv Python explicitly — avoids pyenv/shims taking over on Windows)
```powershell
.venv\Scripts\python.exe -m pip install -r requirements.txt
```

> **Note:** If you use **pyenv-win**, `activate` + `python` may still resolve to the global shim. Prefer `.venv\Scripts\python.exe` or the `.bat` launchers below. If you moved the project folder after creating `.venv`, recreate it with `setup_and_run.py`.
...
```text
run_windows.bat    # uses .venv\Scripts\python.exe (recommended)
run_app.bat        # same; falls back to PATH python if .venv is missing
```
...
### Windows packaging (executable + installer)

1. Active virtual environment and `pip install -r requirements.txt`.
2. (Recommended) Bundle FFmpeg in the build:
   ```powershell
   powershell -ExecutionPolicy Bypass -File scripts\fetch_ffmpeg_windows.ps1
   ```
3. Generate `dist\ClipMaster\` and, if you have [Inno Setup 6](https://jrsoftware.org/isdl.php), `dist_installer\ClipMaster_Setup_*.exe`:
   ```powershell
   powershell -ExecutionPolicy Bypass -File scripts\build_windows_release.ps1
   ```
   Non-interactive / CI: add `-SkipFFmpegCheck -Force`. PyInstaller only: `-SkipInno`.

**End user:** with the installer, the app goes to `%LocalAppData%\Programs\ClipMaster\`; exports and history live under `%LocalAppData%\AI_Viral_Clipper\`.
...
```
clipmaster/
├── main.py                 # Launcher (multiprocessing freeze_support)
├── setup_and_run.py        # First-time venv + dependencies + run
├── run_windows.bat         # Windows quick start (.venv python)
├── run_app.bat
├── pyproject.toml
├── requirements.txt
├── app/
...
│   │   ├── ffmpeg_bin.py   # FFmpeg discovery / bundled binaries
│   │   └── cuda_setup.py   # CUDA/DLL setup
│   ├── models/
│   │   └── schemas.py      # Data types (Clip, Metrics, etc.)
│   ├── services/
│   │   ├── transcription.py    # Whisper — audio to text
│   │   ├── llm_analyzer.py     # LLM analysis (Ollama / Gemini / Groq / …)
│   │   ├── video_engine.py     # FFmpeg — cutting videos
...
MIN_CLIP_SECONDS      # 30s (minimum clip duration)
...
```python
class Clip:
    start: float          # Start second (e.g. 10.5)
    end: float            # End second (e.g. 55.0)
    reason: str           # Why is it viral? (e.g. "Funny moment")
    headline: str         # Short title of the clip

    @property
    def duration:         # Calculates end - start
```
...
transcription_time       # How long transcription took
analysis_time            # How long the AI analysis took
rendering_time           # How long rendering took
total_clips_found        # How many clips were found
clips_selected           # How many the user selected
video_duration           # Duration of the original video
model_used               # e.g. "llama3.2:3b"
prompt_type              # e.g. "Humor and Comedy"
```
...
- **Optional parameter**: `model_name` (from the UI selector)
- **Output**: List of segments + total duration
```python
segments = [
...
- **Humor & Comedy**: Looks for funny moments
- **Serious & High Value**: Looks for educational/valuable content

#### `analyze_viral_potential(text, model, prompt_type)`
Sends text to the LLM and receives clip candidates:
```python
# Input: 10-minute transcript
# Output: List of clips with times and reasons
[
...
```python
# If clip < 30s → expand (add seconds before/after)
# If clip > 60s → trim to exactly 60s
```

#### `remove_duplicate_clips(clips)`
Removes clips that overlap > 50%:
```python
# If two clips share >50% of the same span → remove the weaker one
```

#### `append_history_entry(metrics, video_path, history_file)`
Saves processing history as JSON:
```json
[
  {
...
#### `VideoProcessorThread` (inherits from QThread)
...
thread = VideoProcessorThread(
    ...
    whisper_model="small",
    output_dir="/exports/my_video_processed",
    prompt_type="Humor and Comedy",
    resolution="1080p",
    bitrate="",            # Empty = use CRF 18
...
finished_signal.emit("Processing completed")   # Done
error_signal.emit("ERROR!")                    # Failed
clips_ready_signal.emit(clips_list)            # Clips ready
```
...
5. 🤖 Analyze each chunk with the LLM
6. 🧹 Remove duplicates
7. 📐 Enforce duration limits (30–60s)
8. 💾 Save history
9. 🎬 Render all clips (FFmpeg)

### 9. **ui/main_window.py** — graphical interface

#### `ViralApp` (inherits from QMainWindow)
...
_on_save_clips_clicked()       # Save selected clips
...
**Connected signals:**
```python
# When the thread emits, the UI updates:
...
### 10. **ui/components/drop_zone.py** — drag zone

```python
class DropZone(QLabel):
    file_dropped = Signal(str)  # Emits when a video is dropped

    # Allows dragging a video directly onto the widget
```
...
class ClipSelectionDialog(QDialog):
    # Shows preview of found clips
    # User selects which ones to export
...
```
1️⃣ User runs main.py (or run_windows.bat)
        ↓
...
MAX_CLIP_SECONDS = 60.0  # YouTube Shorts maximum
```

### Chunking (config.py)
```python
CHUNK_SECONDS = 600.0  # 10 minutes per chunk
# 1 hour video = 6 LLM requests
...
```powershell
cd path\to\clipmaster
.venv\Scripts\python.exe main.py
```

### Error: `ModuleNotFoundError: No module named 'PySide6'` (Windows + pyenv)
**Cause:** `activate` + `python` may use **pyenv shims** instead of `.venv`.

**Solution:** Use the launchers or the venv interpreter directly:
```powershell
run_windows.bat
# or
.venv\Scripts\python.exe -m pip install -r requirements.txt
...
If you **moved** the project after creating `.venv`, delete `.venv` and run `python setup_and_run.py` again.

### Error: `ffmpeg not found`
**Solution:** Install FFmpeg and add it to PATH, or bundle binaries under `bundled/ffmpeg/windows/`:
```powershell
...
ollama serve
```
Console warnings about Ollama when it is not installed are normal; the UI still opens. Local LLM features need Ollama installed and on PATH.

### Transcription stalls or fails near the end (GPU / CUDA)
With **Ollama** and **Faster-Whisper** on the **same GPU**, VRAM may run out on the last audio segment (OOM). The app retries on **CPU** automatically; to avoid the failure from the start:

```powershell
# Before opening the app — Whisper on CPU only (Ollama can stay on GPU):
$env:WHISPER_SHARED_GPU_SAFE="1"
.venv\Scripts\python.exe main.py
```

Other options: `WHISPER_DEVICE=cpu`, or `WHISPER_CUDA_LOW_VRAM=1` (keeps CUDA with less VRAM).  
`WHISPER_COMPUTE_TYPE` can still be set manually (e.g. `int8_float16`).

### Whisper very slow
**Reason:** Using CPU  
**Solution:** Install CUDA if you have an NVIDIA GPU

### Poor clip quality
**Solution:** Try other prompt types in the UI, or a custom prompt in `llm_analyzer.py`
...
1. **For better quality:** use `-crf 15–18` (slower, better)
2. **For more clips:** try “Serious & High Value” (finds more content)
3. **For long videos:** use 10-minute chunks for better analysis
4. **History:** check `processing_history.json` for statistics
5. **Customize:** edit prompts in `llm_analyzer.py` for your style
